...
export default async function DashboardPage() {
  ...
  const primary: Tile[] = [
    ...
    },
    {
      ...
      label: 'Enquiries',
      value: counts.leads,
      note: counts.leads > 0 ? 'Rows in the leads collection' : 'No leads captured yet',
      icon: IconInbox,
    },
    {
      key: 'blogs',
      label: 'Articles',
      value: counts.blogs,
      note: counts.blogs > 0 ? 'Rows in the blog collection' : 'No articles published yet',
      icon: IconFile,
    },
  ];
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        actions={
          <Link href="/microsites" className="btn-primary btn-sm">
      ...
      <section aria-label="Catalogue depth" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.25fr_1fr]">
        <div className="card animate-fade-up p-5" style={{ animationDelay: '440ms' }}>
          <h2 className="section-title">Jump back in</h2>
          <p className="muted mt-1">Everything this panel manages today.</p>
        ...
        <div className="card animate-fade-up p-5" style={{ animationDelay: '520ms' }}>
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-600">
            <li className="flex gap-2.5">
              ...
              <span>
                Relationships run through the original MySQL keys stored as strings —{' '}
            ...
            <li className="flex gap-2.5">
              <span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-navy-300" />
              <span>
                Queries use the singular, snake_case collections:{' '}
                <code className="code-chip">microsite</code>, never <code className="code-chip">microsites</code>.
              </span>
            </li>
            <li className="flex gap-2.5">
              <span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-navy-300" />
              <span>
                Passwords are bcrypt-hashed on first login; legacy plaintext rows upgrade themselves.
              </span>
            </li>
          </ul>
        </div>
  ...
  const body = (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {tile.href && (
          <IconChevronRight className="mt-1 h-4 w-4 text-slate-300" aria-hidden="true" />
        )}
      </div>
      ...
      >
        <CountUp value={tile.value} delay={index * 60} />
      </p>
